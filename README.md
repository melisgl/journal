...
[`JOURNALED`][4f52] are simply written to a journal, which is a sequence of
events much like a file. What events are generated is described in
[`JOURNALED`][4f52]. [Replay][0dc7] is much more involved, thus it gets its own
...
- [macro] **WITH-JOURNALING** *(&KEY RECORD REPLAY REPLAY-EOJ-ERROR-P) &BODY BODY*
    ...
    the `LOG-RECORD` argument of [`JOURNALED`][4f52]). If `REPLAY` designates a
    ...
    [`JOURNAL-STATE`][3631] is not `:NEW`), or if `REPLAY` is a [`JOURNAL`][86bc] that is not a
    complete recording of successful replay (i.e. its [`JOURNAL-STATE`][3631] is
    ...
    `:FAILED` in its [`JOURNAL-STATE`][3631].
...
- [macro] **JOURNALED** *(NAME &KEY (LOG-RECORD :RECORD) VERSION ARGS VALUES CONDITION INSERTABLE REPLAY-VALUES REPLAY-CONDITION) &BODY BODY*
    ...
    [`WITH-JOURNALING`][234e] and `LOG-RECORD` is `NIL`, then event recording is
...
from the `NAME`, `VERSION` and `ARGS` arguments of [`JOURNALED`][4f52]. For example,
...
[`JOURNALED`][4f52]. `EXIT` and `OUTCOME` are filled in differently depending on
...
- [glossary-term] **values outcome**
    ...
    argument of [`JOURNALED`][4f52], whose default is `#'IDENTITY`. Also see
...
- [glossary-term] **condition outcome**
    ...
    Furthermore, given a suitable `REPLAY-CONDITION` in [`JOURNALED`][4f52], they
...
- [glossary-term] **error outcome**
    ...
    [`JOURNALED`][4f52]'s `CONDITION` argument returns `NIL` when invoked on it, then
...
argument of [`JOURNALED`][4f52] is for.
...
- [condition] **JOURNALING-FAILURE** *SERIOUS-CONDITION*
    ...
    encountered reading or writing journals by [`WITH-JOURNALING`][234e],
    ...
    [`JOURNALED`][4f52]'s `VALUES` and `CONDITION` function arguments. Crucially, this
    does not apply to non-local exits from other code, such as [`JOURNALED`][4f52]
    ...
    handled within [`WITH-JOURNALING`][234e].
    ...
    [`WITH-JOURNALING`][234e] to use the critical machinery mentioned
    above ([`JOURNALED`][4f52], [`LOGGED`][0765], etc) resignals the same journal failure
    condition. As a consequence, the record journal cannot be changed
    and the only way to recover is to leave [`WITH-JOURNALING`][234e]. This does
    ...
    which necessitate leaving [`WITH-JOURNALING`][234e] to recover from, the other
...
- [condition] **RECORD-UNEXPECTED-OUTCOME**
    ...
    signalling this condition, [`JOURNAL-STATE`][3631] is set to `:LOGGING`, thus no
...
[`LOGGED`][0765] is for single messages. [`JOURNALED`][4f52] can provide nested context:
...
- [glossary-term] **decoration**
    ...
    can be on [`LOG-EVENT`][3a51]s only, do not affect [Replay][0dc7]. Decorations are
...
The `LOG-RECORD` argument of [`JOURNALED`][4f52] and [`LOGGED`][0765] controls where
[`LOG-EVENT`][3a51]s are written both within [`WITH-JOURNALING`][234e] and without. The
...
[`WITH-JOURNALING`][234e], and it does not affect the [`JOURNAL-STATE`][3631]. However,
...
- [macro] **LOGGED** *(&OPTIONAL (LOG-RECORD :RECORD)) FORMAT-CONTROL &REST FORMAT-ARGS*
    ...
    [`LEAF-EVENT`][fef8]s are [`LOG-EVENT`][3a51]s with no separate in- and out-events. They
...
when [`WITH-JOURNALING`][234e] finishes, it might signal [`REPLAY-INCOMPLETE`][32a1] if
...
automated by [Bundles][5b0f]. [`WITH-JOURNALING`][234e] does not allow replay from
...
- [type] **JOURNAL-STATE**
    ...
    - **`:LOGGING`**: There was a [`RECORD-UNEXPECTED-OUTCOME`][cf72]. In this
      state, [`VERSIONED-EVENT`][ffc4]s generated are downgraded to [`LOG-EVENT`][3a51]s,
      [`EXTERNAL-EVENT`][eed7]s and [Invoked][4492] trigger a [`DATA-EVENT-LOSSAGE`][1668].
    ...
    - **`:COMPLETED`**: All events were successfully replayed and
      [`WITH-JOURNALING`][234e] finished or a [`JOURNALING-FAILURE`][6db0] occurred while
...
- `INSERTABLE` controls whether [`VERSIONED-EVENT`][ffc4]s and [`EXTERNAL-EVENT`][eed7]s
  ...
  \_insert\_ed. Note that inserting [`EXTERNAL-EVENT`][eed7]s while `:REPLAYING`
  ...
  to properly insert these kinds of [`EXTERNAL-EVENT`][eed7]s.
...
- [glossary-term] **Invoked**
    ...
    [`JOURNALED`][4f52] and its wrappers put code first, and the journal will be a
...
- [macro] **DEFINE-INVOKED** *FUNCTION-NAME ARGS (NAME &KEY (VERSION 1) INSERTABLE) &BODY BODY*
    ...
    positional arguments are allowed) and wraps [`CHECKED`][cc3d] with `NAME`, the
    same `ARGS` and `INSERTABLE` around `BODY`. Whenever an [`IN-EVENT`][e10e] becomes
...
- [macro] **FLET-INVOKED** *DEFINITIONS &BODY BODY*
    ...
    [`DEFINE-INVOKED`][635d] can be rewritten as:
   ...
   advancing the replay journal).
    ...
    - **mismatched name**: Else, if the next unread replay event's
      name is not `EQUAL` to the name of the new event, then:
        ...
        - For [`VERSIONED-EVENT`][ffc4]s, **[`REPLAY-NAME-MISMATCH`][d031]** is
          signalled if `INSERTABLE` is `NIL`, else **insert** is
          returned.

        - For [`EXTERNAL-EVENT`][eed7]s, **[`REPLAY-NAME-MISMATCH`][d031]** is
          signalled.
...
- At this point, two things might happen:
    ...
    - For [`EXTERNAL-EVENT`][eed7]s, the corresponding replay [`OUT-EVENT`][cba8] is
      ...
      replayed (see [Replaying the outcome][b6d1]). If the [`OUT-EVENT`][cba8] is
      missing, then [`EXTERNAL-EVENT`][eed7]s behave like [`VERSIONED-EVENT`][ffc4]s,
...
- Similarly, if the replay event has unwound with an expected
  condition (has `EVENT-EXIT`([`0`][a6d1] [`1`][306a]) `:CONDITION`), then the recorded
...
- [condition] **REPLAY-FAILURE** *SERIOUS-CONDITION*
    ...
    to [`JOURNAL-STATE`][3631] `:MISMATCHED`. Like [`JOURNALING-FAILURE`][6db0], this is a
    serious condition because it is to be handled outside the enclosing
    [`WITH-JOURNALING`][234e]. If a `REPLAY-FAILURE` were to be handled inside the
    [`WITH-JOURNALING`][234e], keep in mind that in `:MISMATCHED`, replay always
...
- [condition] **REPLAY-INCOMPLETE** *REPLAY-FAILURE*
    ...
    [`WITH-JOURNALING`][234e] returned normally, which is to prevent this
...
- We can insert individual frames with [`JOURNALED`][4f52]'s `INSERTABLE`,
  upgrade frames by bumping [`JOURNALED`][4f52]'s `VERSION`, and filter frames
...
- Or we may decide to keep the bare minimum of the replay journal
  around, and discard everything except for [`EXTERNAL-EVENT`][eed7]s. This
...
- Rerecording the journal without replay might be another option if
  there are no [`EXTERNAL-EVENT`][eed7]s to worry about.
...
With that, let's see how [`WITH-REPLAY-FILTER`][fa00] works.
...
- [function] **PEEK-REPLAY-EVENT** 
    ...
    `:MAP`, and it may return `(:INDETERMINATE)` if [`WITH-REPLAY-FILTER`][fa00]
...
- [macro] **WITH-REPLAY-FILTER** *(&KEY MAP SKIP NO-REPLAY-OUTCOME) &BODY BODY*
    ...
    - `SKIP`: In addition to filtering out [`LOG-EVENT`][3a51]s (which always
      ...
      out events are never seen by [`JOURNALED`][4f52] as it replays events. `SKIP`
      patterns are of the format `(&KEY NAME VERSION<)`, where `VERSION`<
      is a valid [`EVENT-VERSION`][917c], and `NAME` may be `NIL`, which acts
      as a wildcard.
    
        `SKIP` is for when [`JOURNALED`][4f52] [block][6572]s are removed from the code,
    ...
    same [`WITH-JOURNALING`][234e] inherits the `SKIP` patterns of its parent, to
...
of the external [`JOURNALED`][4f52] blocks are replayed from the journal:
...
suitable for further processing. Suppose we wrap [`REPLAYED`][c2fb] around
...
outer [`REPLAYED`][c2fb] from being replayed by outcome with
...
in [`REPLAYED`][c2fb], which allows it to reconstruct the state of the program
...
- `T`: This is the *no data loss* setting with minimal
  ...
  [`JOURNAL-STATE`][3631] `:RECORDING` or for the entire record journal in
...
[Streamlets reference][2453]). Using [`JOURNALED`][4f52] wrapped in a
[`WITH-JOURNALING`][234e], [`WITH-BUNDLE`][0ddc], or [Log record][3380] without [`WITH-JOURNALING`][234e]
is thread-safe.

- Every journal is guaranteed to have at most a single writer active
  at any time. Writers are [`WITH-JOURNALING`][234e] and [`WITH-BUNDLE`][0ddc], but also
...
- [`WITH-JOURNALING`][234e] and [`WITH-BUNDLE`][0ddc] have dynamic extent as writers,
...
- The juggling of replay and record journals performed by
  [`WITH-BUNDLE`][0ddc] is also thread-safe.
...
So a [`JOURNALED`][4f52] [block][6572] generates an [`IN-EVENT`][e10e] and an [`OUT-EVENT`][cba8], which
...
- [function] **SYNC-JOURNAL** *&OPTIONAL (JOURNAL (RECORD-JOURNAL))*
    ...
    made in a [`WITH-JOURNALING`][234e] in another thread are not persisted. The
...
- [class] **FILE-JOURNAL** *[JOURNAL][86bc]*
    ...
    - The first character of the file represents its [`JOURNAL-STATE`][3631]. It

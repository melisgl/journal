...
single macro at its heart: [`JOURNALED`][a1aa], which does pretty much what
...
The [`JOURNALED`][a1aa] macro does both recording and replaying of events,
possibly at the same time. Recording is easy: events generated by
[`JOURNALED`][a1aa] are simply written to a journal, which is a sequence of
events much like a file. What events are generated is described in
[`JOURNALED`][a1aa]. [Replay][0dc7] is much more involved, thus it gets its own
section. The journals used for recording and replaying are specified
by [`WITH-JOURNALING`][c7e8] or by [`WITH-BUNDLE`][4522].
...
<a id='x-28JOURNAL-3AWITH-JOURNALING-20-28MGL-PAX-3AMACRO-29-29'></a>
...
- [macro] **WITH-JOURNALING** *(&KEY RECORD REPLAY REPLAY-EOJ-ERROR-P) &BODY BODY*
    ...
    If `RECORD` designates a [`JOURNAL`][86bc], then events generated by enclosed
    [`JOURNALED`][a1aa] [block][6572]s are written to that journal (with exceptions, see
    the `LOG-RECORD` argument of [`JOURNALED`][a1aa]). If `REPLAY` designates a
    [`JOURNAL`][86bc], then the generated events are matched against events from
    that journal according to the rules of [Replay][0dc7].
    
    A [`JOURNAL-ERROR`][571f] is signalled, if `RECORD` is a [`JOURNAL`][86bc] that has been
    previously recorded to by another [`WITH-JOURNALING`][c7e8] (that is, if its
    [`JOURNAL-STATE`][7715] is not `:NEW`), or if `REPLAY` is a [`JOURNAL`][86bc] that is not a
    complete recording of successful replay (i.e. its [`JOURNAL-STATE`][7715] is
    ...
    [`WITH-JOURNALING`][c7e8] finishes, the `RECORD` journal is marked `:COMPLETED` or
    `:FAILED` in its [`JOURNAL-STATE`][7715].
    ...
    or a [`REPLAY-FAILURE`][955f] during execution, then [`END-OF-JOURNAL`][9c7e] is not
    signalled.
    
    If `BODY` completes successfully, but `REPLAY` has unprocessed events,
    then signal [`REPLAY-INCOMPLETE`][32a1].
    
    [`WITH-JOURNALING`][c7e8] for different `RECORD` journals can be nested and run
    independently.
...
- [glossary-term] **block**
    ...
    [`JOURNALED`][a1aa]. When a block is executed, a [frame][1452] is created.

<a id='x-28JOURNAL-3A-40FRAME-20MGL-PAX-3AGLOSSARY-TERM-29'></a>

- [glossary-term] **frame**

    A frame is an [`IN-EVENT`][d83b], [`OUT-EVENT`][506d] pair, which are created when a
...
- [function] **RECORD-JOURNAL** 
    ...
    recorded (see [`WITH-JOURNALING`][c7e8] and [`WITH-BUNDLE`][4522]) or `NIL`.
...
- [function] **REPLAY-JOURNAL** 
    ...
    replayed (see [`WITH-JOURNALING`][c7e8] and [`WITH-BUNDLE`][4522]) or `NIL`.

<a id='x-28JOURNAL-3AJOURNALED-20-28MGL-PAX-3AMACRO-29-29'></a>
...
- [macro] **JOURNALED** *(NAME &KEY (LOG-RECORD :RECORD) VERSION ARGS VALUES CONDITION INSERTABLE REPLAY-VALUES REPLAY-CONDITION) &BODY BODY*
    ...
    [`WITH-JOURNALING`][c7e8] and `LOG-RECORD` is `NIL`, then event recording is
    turned off and [`JOURNALED`][a1aa] imposes minimal overhead.
...
Upon entering a [block][6572], [`JOURNALED`][a1aa] generates an [`IN-EVENT`][d83b],
which conceptually opens a new [frame][1452]. These in-events are created
from the `NAME`, `VERSION` and `ARGS` arguments of [`JOURNALED`][a1aa]. For example,
...
Upon leaving a [block][6572], [`JOURNALED`][a1aa] generates and [`OUT-EVENT`][506d], closing
...
[`JOURNALED`][a1aa]. `EXIT` and `OUTCOME` are filled in differently depending on
how the block finished its execution.

<a id='x-28JOURNAL-3AEVENT-EXIT-20-28TYPE-29-29'></a>
...
- [glossary-term] **values outcome**
    ...
    If the [`JOURNALED`][a1aa] [block][6572] returns normally, [`EVENT-EXIT`][ef19] is
    ...
    argument of [`JOURNALED`][a1aa], whose default is `#'IDENTITY`. Also see
...
- [glossary-term] **condition outcome**
    ...
    If the [block][6572] unwound due to a condition, and [`JOURNALED`][a1aa]'s
    `CONDITION` argument (a function whose default is `(CONSTANTLY NIL)`)
    returns non-NIL when invoked on it, then [`EVENT-EXIT`][ef19] is
    ...
    Furthermore, given a suitable `REPLAY-CONDITION` in [`JOURNALED`][a1aa], they
    may be replayed without running the [block][6572].

<a id='x-28JOURNAL-3AERROR-OUTCOME-20MGL-PAX-3AGLOSSARY-TERM-29'></a>

- [glossary-term] **error outcome**

    If the [`JOURNALED`][a1aa] [block][6572] unwound due to a condition, but
    [`JOURNALED`][a1aa]'s `CONDITION` argument returns `NIL` when invoked on it, then
    [`EVENT-EXIT`][ef19] is `:ERROR` and the outcome the string
...
- [glossary-term] **nlx outcome**
    ...
    If the [`JOURNALED`][a1aa] [block][6572] performed a [non-local exit][e17e] that was not
    due to a condition, then [`EVENT-EXIT`][ef19] is `:NLX` and the outcome
    is `NIL`.
    ...
    [non-local exit][e17e]s, but are distinct from nlx outcomes.
...
journal, this is not a problem in practice. [`JOURNALED`][a1aa] provides the
`VALUES` hook for this purpose.

With [`EXTERNAL-EVENT`][093c]s, whose outcome is replayed (see
...
argument of [`JOURNALED`][a1aa] is for.
...
something [readable][531d]. On the `Recording` run, `(VALUES-> #'USER-ID)`
replaces the user object with its id in the [`EVENT-OUTCOME`][95b8] recorded,
but the original user object is returned.

When `Replaying`, the journaled [`OUT-EVENT`][506d] is replayed (see
...
- [function] **VALUES-\>** *&REST FNS*
    ...
    [`JOURNALED`][a1aa]. The `VALUES` function is called with the list of values
    ...
    allowed, [`VALUES->`][a799] handles the common case of transforming
...
- [function] **VALUES\<-** *&REST FNS*
    ...
    the `REPLAY-VALUES` argument of [`JOURNALED`][a1aa]. It does pretty much what
    [`VALUES->`][a799] does, but the function returned returns the transformed
...
- [function] **EVENTS-TO-FRAMES** *EVENTS*
    ...
    an [`OUT-EVENT`][506d]) are included in the output.
...
- [function] **PPRINT-EVENTS** *EVENTS &KEY STREAM (PRETTIFIER 'PRETTIFY-EVENT)*
    ...
    description of `PRETTIFIER`'s arguments see [`PRETTIFY-EVENT`][29db].
...
- [function] **PRETTIFY-EVENT** *EVENT DEPTH STREAM*
    ...
    default. In addition to the basic example in [`PPRINT-EVENTS`][74d2],
    ...
    0. [`PRETTIFY-EVENT`][29db] prints indents the output, after printing the
...
Note that [Pretty-printing journals][eeda] are not tied to [`WITH-JOURNALING`][c7e8] and are
most often used for [Logging][77df] and [Tracing][7849].

<a id='x-28JOURNAL-3A-40ERROR-HANDLING-20MGL-PAX-3ASECTION-29'></a>
...
- [condition] **JOURNALING-FAILURE** *SERIOUS-CONDITION*
    ...
    [`WITH-JOURNALING`][c7e8] when an error threatens to leave the journaling
    mechanism in an inconsistent state. These include I/O errors
    encountered reading or writing journals by [`WITH-JOURNALING`][c7e8],
    [`JOURNALED`][a1aa], [`LOGGED`][6830], [`WITH-REPLAY-FILTER`][eeec], [`SYNC-JOURNAL`][6bc6], but also
    `STORAGE-CONDITION`s, assertion failures, and errors calling
    [`JOURNALED`][a1aa]'s `VALUES` and `CONDITION` function arguments. Crucially, this
    does not apply to non-local exits from other code, such as [`JOURNALED`][a1aa]
    ...
    [non-local exit][e17e] like `THROW`. This is a `SERIOUS-CONDITION`, not to be
    handled within [`WITH-JOURNALING`][c7e8].
    ...
    out-of-sync. The [`RECORD-JOURNAL`][35c4] may have missing events (or even
    ...
    upon signalling `JOURNALING-FAILURE`, [`JOURNAL-STATE`][7715] is set to
    ...
    [`WITH-JOURNALING`][c7e8] to use the critical machinery mentioned
    above ([`JOURNALED`][a1aa], [`LOGGED`][6830], etc) resignals the same journal failure
    condition. As a consequence, the record journal cannot be changed
    and the only way to recover is to leave [`WITH-JOURNALING`][c7e8]. This does
    ...
    which necessitate leaving [`WITH-JOURNALING`][c7e8] to recover from, the other
...
- [condition] **RECORD-UNEXPECTED-OUTCOME**
    ...
    [`JOURNALED`][a1aa] when a [`VERSIONED-EVENT`][e361] or an [`EXTERNAL-EVENT`][093c] had an
    [unexpected outcome][f57e] while in [`JOURNAL-STATE`][7715] `:RECORDING`. Upon
...
- [condition] **JOURNAL-ERROR** *ERROR*
    ...
    Signalled by [`WITH-JOURNALING`][c7e8], [`WITH-BUNDLE`][4522] and by
...
- [condition] **END-OF-JOURNAL** *JOURNAL-ERROR*
    ...
    [`WITH-JOURNALING`][c7e8]'s `REPLAY-EOJ-ERROR-P` is true. Unlike
    [`REPLAY-FAILURE`][955f]s, this does not affect [`JOURNAL-STATE`][7715] of
...
Note how pretty-printing was turned off and we see the [`LEAF-EVENT`][c905]
generated by [`LOGGED`][6830] in its raw plist form.
...
[`LOGGED`][6830] is for single messages. [`JOURNALED`][a1aa] can provide nested context:
...
- [glossary-term] **decoration**
    ...
    [`JOURNAL-LOG-DECORATOR`][d3f7] adds additional data to [`LOG-EVENT`][4d31]s as they
    ...
    can be on [`LOG-EVENT`][4d31]s only, do not affect [Replay][0dc7]. Decorations are
...
- [accessor] **JOURNAL-LOG-DECORATOR** *JOURNAL* *(:LOG-DECORATOR = NIL)*
    ...
    [`LOG-EVENT`][4d31]s before they are written to a journal. The only allowed
...
[`WITH-JOURNALING`][c7e8] and [`WITH-BUNDLE`][4522] control replaying and recording
within their dynamic extent, which is rather a necessity because
[Replay][0dc7] needs to read the events in the same order as the [`JOURNALED`][a1aa]
[block][6572]s are being executed. However, [`LOG-EVENT`][4d31]s do not affect replay
so we can allow more flexibility in routing them.

The `LOG-RECORD` argument of [`JOURNALED`][a1aa] and [`LOGGED`][6830] controls where
[`LOG-EVENT`][4d31]s are written both within [`WITH-JOURNALING`][c7e8] and without. The
...
[`WITH-JOURNALING`][c7e8], and it does not affect the [`JOURNAL-STATE`][7715]. However,
it is a [`JOURNAL-ERROR`][571f] to write to a `:COMPLETED` journal (see
[`JOURNAL-STATE`][7715]).
...
<a id='x-28JOURNAL-3ALOGGED-20-28MGL-PAX-3AMACRO-29-29'></a>

- [macro] **LOGGED** *(&OPTIONAL (LOG-RECORD :RECORD)) FORMAT-CONTROL &REST FORMAT-ARGS*

    [`LOGGED`][6830] creates a single [`LEAF-EVENT`][c905] whose name is the string
    ...
    [`LEAF-EVENT`][c905]s are [`LOG-EVENT`][4d31]s with no separate in- and out-events. They
    have an [`EVENT-NAME`][2b45] and no other properties. Use [`LOGGED`][6830] for
    point-in-time textual log messages, and [`JOURNALED`][a1aa] with `VERSION`
    `NIL` (i.e. [`FRAMED`][6505]) to provide context.
...
[`JTRACE`][6519] behaves similarly to `CL:TRACE`, but deals [non-local exit][e17e]s
gracefully.
...
In the above, [`*TRACE-JOURNAL*`][87c4] was bound locally to keep the example
...
<a id='x-28JOURNAL-3AJTRACE-20-28MGL-PAX-3AMACRO-29-29'></a>

- [macro] **JTRACE** *&REST NAMES*

    Like `CL:TRACE`, [`JTRACE`][6519] takes a list of symbols. When functions
    ...
    values may not be [readable][531d], [`JTRACE`][6519] does not care.
    
    The format of the output is the same as that of [`PPRINT-EVENTS`][74d2].
    Behind the scenes, [`JTRACE`][6519] encapsulates the global functions with
    ...
    If [`JTRACE`][6519] is invoked with no arguments, it returns the list of
    symbols currently traced.
    
    On Lisps other than SBCL, where a function encapsulation facility is
    not available or it is not used by Journal, [`JTRACE`][6519] simply sets
    ...
    The main advantage of [`JTRACE`][6519] over `CL:TRACE` is the ability to trace
    errors, not just normal return values. As it is built on [`JOURNALED`][a1aa],
    it can also detect - somewhat heuristically - `THROW`s and similar.

<a id='x-28JOURNAL-3AJUNTRACE-20-28MGL-PAX-3AMACRO-29-29'></a>

- [macro] **JUNTRACE** *&REST NAMES*

    Like `CL:UNTRACE`, [`JUNTRACE`][36ac] makes it so that the global functions
    denoted by the symbols `NAMES` are no longer traced by [`JTRACE`][6519]. When
    invoked with no arguments, it untraces all traced functions.

<a id='x-28JOURNAL-3A-2ATRACE-PRETTY-2A-20-28VARIABLE-29-29'></a>

- [variable] **\*TRACE-PRETTY\*** *T*

    If `*TRACE-PRETTY*` is true, then [`JTRACE`][6519] produces output like
    [`PPRINT-EVENTS`][74d2], else it's like [`PRINT-EVENTS`][3bd9].

<a id='x-28JOURNAL-3A-2ATRACE-THREAD-2A-20-28VARIABLE-29-29'></a>
...
<a id='x-28JOURNAL-3A-2ATRACE-TIME-2A-20-28VARIABLE-29-29'></a>
...
<a id='x-28JOURNAL-3A-2ATRACE-REAL-TIME-2A-20-28VARIABLE-29-29'></a>
...
<a id='x-28JOURNAL-3A-2ATRACE-RUN-TIME-2A-20-28VARIABLE-29-29'></a>
...
<a id='x-28JOURNAL-3A-2ATRACE-JOURNAL-2A-20-28VARIABLE-29-29'></a>

- [variable] **\*TRACE-JOURNAL\*** *#<PPRINT-JOURNAL :NEW>*

    The [`JOURNAL`][86bc] where [`JTRACE`][6519] writes [`LOG-EVENT`][4d31]s. By default it is a
    [`PPRINT-JOURNAL`][2123] that sets up a `SYNONYM-STREAM` to `*TRACE-OUTPUT*` and
    sends its output there. It pays attention to [`*TRACE-PRETTY*`][e873], and its
    log decorator is affected by [`*TRACE-TIME*`][ce5d] and [`*TRACE-THREAD*`][b3dc].
...
`C-c C-t` to toggling `CL:TRACE`. To integrate [`JTRACE`][6519] into Slime, add
...
Since [`JTRACE`][6519] lacks some features of `CL:TRACE`, most notably that of
...
Replay can be only be initiated with [`WITH-JOURNALING`][c7e8] (or its close
kin [`WITH-BUNDLE`][4522]). After the per-event processing described below,
when [`WITH-JOURNALING`][c7e8] finishes, it might signal [`REPLAY-INCOMPLETE`][32a1] if
...
[`REPLAY-FAILURE`][955f]. A journal that records events from a successful
...
automated by [Bundles][5b0f]. [`WITH-JOURNALING`][c7e8] does not allow replay from
journals that were failed replays themselves. The mechanism, in
terms of which tracking success and failure of replays is
implemented, revolves around [`JOURNAL-STATE`][7715] and
[`EVENT-VERSION`][55cf]s, which we discuss next.

<a id='x-28JOURNAL-3AJOURNAL-STATE-20-28TYPE-29-29'></a>

- [type] **JOURNAL-STATE**

    [`JOURNAL`][86bc]'s state, with respect to replay, is updated during
    [`WITH-JOURNALING`][c7e8]. This possible states are:
    ...
    - **`:COMPLETED`**: All events were successfully replayed and
      [`WITH-JOURNALING`][c7e8] finished or a [`JOURNALING-FAILURE`][6db0] occurred while
...
The following arguments of [`JOURNALED`][a1aa] control behaviour under replay.
...
- `REPLAY-CONDITION`, a function or `NIL`, may be called with
  [`EVENT-OUTCOME`][95b8] (the return value of the function provided as
  `CONDITION`) when replaying and `:VERSION` is `:INFINITY`. `NIL` is
  equivalent to `ERROR`. Replaying conditions is cumbersome and best
  avoided.


<a id='x-28JOURNAL-3A-2AFORCE-INSERTABLE-2A-20-28VARIABLE-29-29'></a>
...
- [type] **EVENT-VERSION**
    ...
    which correspond to [`LOG-EVENT`][4d31]s, [`VERSIONED-EVENT`][e361]s, and
    [`EXTERNAL-EVENT`][093c]s, respectively, and have an increasingly strict
...
<a id='x-28JOURNAL-3ALOG-EVENT-20-28TYPE-29-29'></a>

- [type] **LOG-EVENT**

    Events with [`EVENT-VERSION`][55cf] `NIL` called log events. During [Replay][0dc7],
...
- [type] **VERSIONED-EVENT**
    ...
    Events with a positive integer [`EVENT-VERSION`][55cf] are called
    versioned events. In [Replay][0dc7], they undergo consistency checks unlike
    [`LOG-EVENT`][4d31]s, but the rules for them are less strict than for
...
- [type] **EXTERNAL-EVENT**
    ...
    Events with [`EVENT-VERSION`][55cf] `:INFINITY` are called external events.
    They are like [`VERSIONED-EVENT`][e361]s whose version was bumped all the way
...
<a id='x-28JOURNAL-3AWITH-BUNDLE-20-28MGL-PAX-3AMACRO-29-29'></a>

- [macro] **WITH-BUNDLE** *(BUNDLE) &BODY BODY*

    This is like [`WITH-JOURNALING`][c7e8] where the [`REPLAY-JOURNAL`][103b] is the last
    successfully completed one in `BUNDLE`, and the [`RECORD-JOURNAL`][35c4] is a
    new one created in `BUNDLE`. When [`WITH-BUNDLE`][4522] finishes, the record
    journal is in [`JOURNAL-STATE`][7715] `:FAILED` or `:COMPLETED`.
    
    To avoid accumulating useless data, the new record is immediately
    deleted when [`WITH-BUNDLE`][4522] finishes if it has not diverged from the
    ...
    It is a [`JOURNAL-ERROR`][571f] to have concurrent or nested [`WITH-BUNDLE`][4522]s on
    the same bundle.
...
- **insert**: The new event is not matched to any replay event, and
  ...
  empty. This is always the case for new [`LOG-EVENT`][4d31]s and when there
  are no more events to read from the replay journal (unless
  `REPLAY-EOJ-ERROR-P`). For [`VERSIONED-EVENT`][e361]s, it is affected by
  setting [`JOURNALED`][a1aa]'s `INSERTABLE` to true (see
  [Journaled for replay][0bc8]).
...
1. Log events are not matched to the replay. If the new event is a
   ...
   record journal's [`JOURNAL-STATE`][7715] is `:MISMATCHED`), then **insert**
   is returned.
...
2. Else, log events to be read in the replay journal are skipped,
   ...
   advancing the replay journal).
    ...
    - **end of replay**: If there are no replay events left, then:
        ...
        - If `REPLAY-EOJ-ERROR-P` is `NIL` in [`WITH-JOURNALING`][c7e8] (the
          default), **insert** is returned.

        - If `REPLAY-EOJ-ERROR-P` is true, then **[`END-OF-JOURNAL`][9c7e]**
          is signalled.

    - **mismatched name**: Else, if the next unread replay event's
      name is not `EQUAL` to the name of the new event, then:

        - For [`VERSIONED-EVENT`][e361]s, **[`REPLAY-NAME-MISMATCH`][d031]** is
          signalled if `INSERTABLE` is `NIL`, else **insert** is
          returned.

        - For [`EXTERNAL-EVENT`][093c]s, **[`REPLAY-NAME-MISMATCH`][d031]** is
          signalled.
    ...
    - **matching name**: Else, if the name of the next unread event
      ...
      it is chosen as the *replay* event.
        ...
        - If the replay event's version is higher than the new
          event's version, then **[`REPLAY-VERSION-DOWNGRADE`][2efb]** is
          signalled.
...
- At this point, two things might happen:
    ...
    - For [`EXTERNAL-EVENT`][093c]s, the corresponding replay [`OUT-EVENT`][506d] is
      ...
      replayed (see [Replaying the outcome][b6d1]). If the [`OUT-EVENT`][506d] is
      missing, then [`EXTERNAL-EVENT`][093c]s behave like [`VERSIONED-EVENT`][e361]s,
      and the [block][6572] is executed.
...
`EVENT-VERSION`([`0`][4f5f] [`1`][55cf]) is `:INFINITY`, then normal execution is altered in the
following manner:
...
- Similarly, if the replay event has unwound with an expected
  ...
  condition (in [`EVENT-OUTCOME`][95b8]) is signalled as
...
[`IN-EVENT`][d83b], and the conditions for [Replaying the outcome][b6d1] were not
met, then the [block][6572] is executed. When the outcome of the block is
determined, an [`OUT-EVENT`][506d] is triggered and is matched to the replay
...
present in [`REPLAY-JOURNAL`][103b], which may be the case when the recording
process failed hard without unwinding properly, or when an
[unexpected outcome][f57e] triggered the transition to [`JOURNAL-STATE`][7715]
`:LOGGING`.
...
- [condition] **REPLAY-FAILURE** *SERIOUS-CONDITION*
    ...
    journal. Signalled only in [`JOURNAL-STATE`][7715] `:REPLAYING` and only once
    per [`WITH-JOURNALING`][c7e8]. If a `REPLAY-FAILURE` is signalled for an [`EVENT`][efed],
    then the event will be recorded, but [`RECORD-JOURNAL`][35c4] will transition
    to [`JOURNAL-STATE`][7715] `:MISMATCHED`. Like [`JOURNALING-FAILURE`][6db0], this is a
    serious condition because it is to be handled outside the enclosing
    [`WITH-JOURNALING`][c7e8]. If a `REPLAY-FAILURE` were to be handled inside the
    [`WITH-JOURNALING`][c7e8], keep in mind that in `:MISMATCHED`, replay always
...
with [`EXTERNAL-EVENT`][093c]s. However, when the code changes, we might find
...
- Rerecording the journal without replay might be another option if
  there are no [`EXTERNAL-EVENT`][093c]s to worry about.
...
With that, let's see how [`WITH-REPLAY-FILTER`][eeec] works.

<a id='x-28JOURNAL-3AWITH-REPLAY-STREAMLET-20-28MGL-PAX-3AMACRO-29-29'></a>

- [macro] **WITH-REPLAY-STREAMLET** *(VAR) &BODY BODY*

    Open [`REPLAY-JOURNAL`][103b] for reading with [`WITH-OPEN-JOURNAL`][4fde] set the
    [`READ-POSITION`][1548] on it to the event next read by the [Replay][0dc7]
    mechanism (which is never a [`LOG-EVENT`][4d31]). The low-level
    [Reading from streamlets][e099] api is then available to inspect the
    contents of the replay. It is an error if [`REPLAY-JOURNAL`][103b] is `NIL`.
...
We do this below by wrapping external interaction in [`JOURNALED`][a1aa] with
...
of the external [`JOURNALED`][a1aa] blocks are replayed from the journal:
...
Note that when this journal is replayed, new [`VERSIONED-EVENT`][e361]s are
...
<a id='x-28JOURNAL-3ADEFINE-FILE-BUNDLE-TEST-20-28MGL-PAX-3AMACRO-29-29'></a>
...
- [macro] **DEFINE-FILE-BUNDLE-TEST** *(NAME &KEY DIRECTORY (EQUIVALENTP T)) &BODY BODY*
    ...
    function's `BODY` is executed in a [`WITH-BUNDLE`][4522] to guarantee
...
in [`REPLAYED`][3394], which allows it to reconstruct the state of the program
...
events and [`JOURNAL-STATE`][7715] changes are persisted durably. For
...
- `NIL`: Never synchronize. A [`FILE-JOURNAL`][f6b2]'s file may be corrupted on
  [aborted execution][33c1]. In [`IN-MEMORY-JOURNAL`][17a8]s, `SYNC-FN` is never called.
...
- `T`: This is the *no data loss* setting with minimal
  ...
  [`JOURNAL-STATE`][7715] `:RECORDING` or for the entire record journal in
  states `:FAILED` and `:COMPLETED`. `:FAILED` or `:COMPLETED` is guaranteed
  when leaving [`WITH-JOURNALING`][c7e8] at the latest.
...
argument, an [`IN-MEMORY-JOURNAL`][17a8]. `SYNC-FN` is called according to
...
time around. This time it ran without error, a [data event][ecce] was
...
[`RECORD-JOURNAL`][35c4] and its [`JOURNAL-STATE`][7715] will be persisted durably in
...
changing the [`JOURNAL-STATE`][7715]. Both are performed by [`JOURNALED`][a1aa] only
unless the low-level streamlet interface is used (see
[Streamlets reference][2453]). Using [`JOURNALED`][a1aa] wrapped in a
[`WITH-JOURNALING`][c7e8], [`WITH-BUNDLE`][4522], or [Log record][3380] without [`WITH-JOURNALING`][c7e8]
is thread-safe.

- Every journal is guaranteed to have at most a single writer active
  at any time. Writers are [`WITH-JOURNALING`][c7e8] and [`WITH-BUNDLE`][4522], but also
...
- [`WITH-JOURNALING`][c7e8] and [`WITH-BUNDLE`][4522] have dynamic extent as writers,
...
- Writing to the same journal via `LOG-RECORD` from multiple threads
  ...
  atomically. Frames can be interleaved, but these are [`LOG-EVENT`][4d31]s,
  so this does not affect replay.

- The juggling of replay and record journals performed by
  [`WITH-BUNDLE`][4522] is also thread-safe.
...
writing the same [`FILE-JOURNAL`][f6b2] or [`FILE-BUNDLE`][eb5d].
...
dynamic extent of a [`JOURNALED`][a1aa] [block][6572] (see [In-events][1d28] and
[Out-events][5721]) and also by [`LOGGED`][6830]. Apart from being part of the
...
So a [`JOURNALED`][a1aa] [block][6572] generates an [`IN-EVENT`][d83b] and an [`OUT-EVENT`][506d], which
are simple property lists. The following reference lists these
properties, there semantics and the functions to read them.

<a id='x-28JOURNAL-3AEVENT-20-28TYPE-29-29'></a>

- [type] **EVENT**

    An event is either an [`IN-EVENT`][d83b], an [`OUT-EVENT`][506d] or a [`LEAF-EVENT`][c905].
...
- [function] **EVENT=** *EVENT-1 EVENT-2*
    ...
    event. [`EVENT-OUTCOME`][95b8] is not compared when `EVENT-EXIT`([`0`][a6d1] [`1`][ef19]) is `:ERROR` to
...
- [function] **IN-EVENT-P** *EVENT*
    ...
    See if `EVENT` is a [`IN-EVENT`][d83b].

<a id='x-28JOURNAL-3AMAKE-IN-EVENT-20FUNCTION-29'></a>

- [function] **MAKE-IN-EVENT** *&KEY NAME VERSION ARGS*

    Create an [`IN-EVENT`][d83b] with `NAME`, `VERSION` (of type [`EVENT-VERSION`][55cf]) and
...
- [function] **OUT-EVENT-P** *EVENT*
    ...
    See if `EVENT` is an [`OUT-EVENT`][506d].

<a id='x-28JOURNAL-3AMAKE-OUT-EVENT-20FUNCTION-29'></a>

- [function] **MAKE-OUT-EVENT** *&KEY NAME VERSION EXIT OUTCOME*

    Create an [`OUT-EVENT`][506d] with `NAME`, `VERSION` (of type [`EVENT-VERSION`][55cf]),
    `EXIT` (of type [`EVENT-EXIT`][ef19]), and `OUTCOME` as its [`EVENT-NAME`][2b45],
...
- [function] **EVENT-EXIT** *OUT-EVENT*
    ...
    Return how the journaled [block][6572] finished. See [`EVENT-EXIT`][ef19]
...
<a id='x-28JOURNAL-3ALEAF-EVENT-20-28TYPE-29-29'></a>

- [type] **LEAF-EVENT**

    Leaf events are triggered by [`LOGGED`][6830]. Unlike [`IN-EVENT`][d83b]s and
    [`OUT-EVENT`][506d]s, which represent a [frame][1452], leaf events stand alone and
...
- [class] **JOURNAL**
    ...
    of by [`WITH-JOURNALING`][c7e8]. For this reason, the user's involvement with
    journals normally only consists of creating and using them in
    [`WITH-JOURNALING`][c7e8].
...
- [function] **SYNC-JOURNAL** *&OPTIONAL (JOURNAL (RECORD-JOURNAL))*
    ...
    enclosing [`WITH-JOURNALING`][c7e8] or via `LOG-RECORD` from any thread. Writes
    made in a [`WITH-JOURNALING`][c7e8] in another thread are not persisted. The
...
- [class] **IN-MEMORY-JOURNAL** *[JOURNAL][86bc]*
    ...
    generated. They differ from [`FILE-JOURNAL`][f6b2]s in that events written to
    ...
    [`JOURNALED`][a1aa] (i.e. its `NAME`, `ARGS` arguments, but also the return `VALUES`
...
- [function] **MAKE-IN-MEMORY-JOURNAL** *&KEY (EVENTS NIL EVENTSP) STATE (SYNC NIL SYNCP) SYNC-FN*
    ...
    By default, creates an empty [`IN-MEMORY-JOURNAL`][17a8] in [`JOURNAL-STATE`][7715]
...
- [class] **FILE-JOURNAL** *[JOURNAL][86bc]*
    ...
    [`JOURNAL-STATE`][7715] are persisted in a file. This is the [`JOURNAL`][86bc]
    ...
    and `WRITE`, everything that [`JOURNALED`][a1aa] records in events (i.e. its
    ...
    - The first character of the file represents its [`JOURNAL-STATE`][7715]. It
...
- [function] **MAKE-FILE-JOURNAL** *PATHNAME &KEY SYNC*
    ...
    If there is already an existing [`FILE-JOURNAL`][f6b2] backed by the same
    ...
    If there is already an existing [`FILE-JOURNAL`][f6b2] backed by the same
    file, the [`JOURNAL-STATE`][7715] is not `:NEW`, but the file doesn't exist,
    then the existing object is **invalidated**: attempts to write will
    fail with [`JOURNAL-ERROR`][571f]. If the existing journal object is being
    written, then invalidation fails with a [`JOURNAL-ERROR`][571f]. After
    invalidation, a new [`FILE-JOURNAL`][f6b2] object is created.
...
[`WITH-BUNDLE`][4522] automates. Here we provide a reference for the bundle
classes.
...
- [class] **BUNDLE**
    ...
    one, extending it with new events. Upon replay (see [`WITH-BUNDLE`][4522]),
    the latest journal in the bundle in [`JOURNAL-STATE`][7715] `:COMPLETED` plays
...
- [accessor] **MAX-N-FAILED** *BUNDLE* *(:MAX-N-FAILED = 1)*
    ...
    journals of [`JOURNAL-STATE`][7715] `:FAILED` in the bundle exceeds
...
- [accessor] **MAX-N-COMPLETED** *BUNDLE* *(:MAX-N-COMPLETED = 1)*
    ...
    journals of [`JOURNAL-STATE`][7715] `:COMPLETED` in the bundle exceeds
    its value, then some journals (starting with the oldest) are
    deleted.
...
- [function] **MAKE-FILE-BUNDLE** *DIRECTORY &KEY (MAX-N-FAILED 1) (MAX-N-COMPLETED 1) SYNC*
    ...
    If there is already a [`FILE-BUNDLE`][eb5d] with the same directory (according
...
- [class] **STREAMLET**
    ...
    [`JOURNAL`][86bc]. The high-level stuff ([`WITH-JOURNALING`][c7e8], [`JOURNALED`][a1aa], etc) is
...
- [generic-function] **MAKE-STREAMLET-FINALIZER** *STREAMLET*
    ...
    streamlets which are not dynamic-extent, so using [`WITH-OPEN-JOURNAL`][4fde]
    is not appropriate.
...
<a id='x-28JOURNAL-3AWITH-OPEN-JOURNAL-20-28MGL-PAX-3AMACRO-29-29'></a>
...
<a id='x-28JOURNAL-3ASAVE-EXCURSION-20-28MGL-PAX-3AMACRO-29-29'></a>
...
- [method] **PEEK-EVENT** *(STREAMLET STREAMLET)*
    ...
    [`SAVE-EXCURSION`][c777] and [`READ-EVENT`][6ed4].
...
- [generic-function] **REQUEST-COMPLETED-ON-ABORT** *STREAMLET*
    ...
    [`JOURNAL`][86bc] will be in [`JOURNAL-STATE`][7715] `:COMPLETED` when loaded fresh (e.g.
    [`FILE-JOURNAL`][f6b2] from a file). Any previously written events must be
    persisted before making this change. Before
    [`REQUEST-COMPLETED-ON-ABORT`][a8b4] is called, a journal must be reloaded in
    state `:FAILED`.
...
- [glossary-term] **non-local exit**
    ...
    exit. This definition of a non-local exit includes `EVENT-EXIT`([`0`][a6d1] [`1`][ef19])
...
- [glossary-term] **readable**
  ...
  [093c]: #x-28JOURNAL-3AEXTERNAL-EVENT-20-28TYPE-29-29 "(JOURNAL:EXTERNAL-EVENT (TYPE))"
  ...
  [1826]: #x-28JOURNAL-3A-40ERROR-HANDLING-20MGL-PAX-3ASECTION-29 "Error handling"
  ...
  [3394]: #x-28JOURNAL-3AREPLAYED-20-28MGL-PAX-3AMACRO-29-29 "(JOURNAL:REPLAYED (MGL-PAX:MACRO))"
  ...
  [36ac]: #x-28JOURNAL-3AJUNTRACE-20-28MGL-PAX-3AMACRO-29-29 "(JOURNAL:JUNTRACE (MGL-PAX:MACRO))"
  ...
  [4fde]: #x-28JOURNAL-3AWITH-OPEN-JOURNAL-20-28MGL-PAX-3AMACRO-29-29 "(JOURNAL:WITH-OPEN-JOURNAL (MGL-PAX:MACRO))"
  [506d]: #x-28JOURNAL-3AOUT-EVENT-20-28TYPE-29-29 "(JOURNAL:OUT-EVENT (TYPE))"
  ...
  [55cf]: #x-28JOURNAL-3AEVENT-VERSION-20-28TYPE-29-29 "(JOURNAL:EVENT-VERSION (TYPE))"
  ...
  [87c4]: #x-28JOURNAL-3A-2ATRACE-JOURNAL-2A-20-28VARIABLE-29-29 "(JOURNAL:*TRACE-JOURNAL* (VARIABLE))"
  ...
  [a1aa]: #x-28JOURNAL-3AJOURNALED-20-28MGL-PAX-3AMACRO-29-29 "(JOURNAL:JOURNALED (MGL-PAX:MACRO))"
  ...
  [b3dc]: #x-28JOURNAL-3A-2ATRACE-THREAD-2A-20-28VARIABLE-29-29 "(JOURNAL:*TRACE-THREAD* (VARIABLE))"
  ...
  [e361]: #x-28JOURNAL-3AVERSIONED-EVENT-20-28TYPE-29-29 "(JOURNAL:VERSIONED-EVENT (TYPE))"
  [e873]: #x-28JOURNAL-3A-2ATRACE-PRETTY-2A-20-28VARIABLE-29-29 "(JOURNAL:*TRACE-PRETTY* (VARIABLE))"
  ...
  [eeec]: #x-28JOURNAL-3AWITH-REPLAY-FILTER-20-28MGL-PAX-3AMACRO-29-29 "(JOURNAL:WITH-REPLAY-FILTER (MGL-PAX:MACRO))"
  [ef19]: #x-28JOURNAL-3AEVENT-EXIT-20-28TYPE-29-29 "(JOURNAL:EVENT-EXIT (TYPE))"
  [efed]: #x-28JOURNAL-3AEVENT-20-28TYPE-29-29 "(JOURNAL:EVENT (TYPE))"

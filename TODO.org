...
* TODO audit what happens if a file journal is corrupted
Truncation can easily happen on crashes. How does replay handle when
the :COMPLETED marker is there but the contents are corrupted? Audit
the WITH-JOURNALING-FAILURE-ON-NLX design and use.
* checkpoints
